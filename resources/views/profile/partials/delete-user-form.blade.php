<div class="bg-white rounded-xl shadow-lg border border-[#faa21b]/20 p-6">
    <form method="post" action="{{ route('profile.destroy') }}">
        ...
        <div class="flex flex-col sm:flex-row gap-4 pt-4">
            <button type="submit" class="inline-flex items-center justify-center px-6 py-2.5 bg-red-600 text-white rounded-xl font-semibold text-sm shadow-lg hover:shadow-xl transition" @click.prevent="if(confirm('{{ __('Êtes-vous sûr de vouloir supprimer votre compte ?') }}')) $el.closest('form').submit()">
