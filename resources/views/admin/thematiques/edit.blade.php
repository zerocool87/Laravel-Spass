<x-app-layout>
    <x-slot name="header">
        <x-elus-header
            title="{{ __('Modifier la thématique') }}"
            subtitle="{{ $thematique->name }}"
            icon="🏷️"
    ...
    <x-slot name="breadcrumbs">
        <x-breadcrumbs :items="[['label' => __('Accueil'), 'url' => route('elus.dashboard')], ['label' => __('Administration'), 'url' => route('elus.admin.index')], ['label' => __('Thématiques'), 'url' => route('admin.thematiques.index')], ['label' => $thematique->name]]" />
    </x-slot>
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-[#faa21b]/20 p-6 sm:p-8">
                <form action="{{ route('admin.thematiques.update', $thematique) }}" method="POST" class="space-y-5">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <input id="name" name="name" type="text" class="w-full input-orange text-base" value="{{ old('name', $thematique->name) }}" required maxlength="255" />
